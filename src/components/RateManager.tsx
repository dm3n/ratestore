...
export function RateManager() {
  ...
  const handleSave = async (rate: MortgageRate) => {
    try {
      const { error } = await supabase
        .from('mortgage_rates')
        .update({
          lender_name: rate.lender_name,
          lender_type: rate.lender_type,
          rate_type: rate.rate_type,
          term: rate.term,
          base_rate: rate.base_rate,
          min_down_payment: rate.min_down_payment,
          max_loan_to_value: rate.max_loan_to_value,
          transaction_types: rate.transaction_types,
          prime_discount: rate.prime_discount || null,
    ...
    <Card>
      <CardHeader>
        <div className="flex justify-between items-center">
          <CardTitle>Mortgage Rate Manager</CardTitle>
            ...
            <TableBody>
              {rates.map((rate) => (
                <TableRow key={rate.id}>
                  ...
                  <TableCell>{(rate.base_rate * 100).toFixed(2)}%</TableCell>
